...
def generate_graph():
    users = load_json('dataAnalysis/User.txt')
...
def generate_training():
    r = load_json('dataAnalysis/Review.txt', True, True)
...
def generate_testing(r, c, date=None):
    ...
    for i in [1, 2, 3]:
        with open('test_data_q%d.txt' % i, 'w') as fq:
            with open('test_data_a%d.txt' % i, 'w') as fa:
                for k in d:
...
def generate_training2():
    ...
    with open('testing_business.txt', 'w') as f:
        f.write(' '.join(c))
...
def generate_user_location():
    test_business = 'dataAnalysis/testing_business.txt'
    with open(test_business, 'r') as f:
        tb = f.read().strip().split()

    r = load_json('dataAnalysis/Review.txt', True, True)
    b = load_json('dataAnalysis/Business.txt')
    ...
    for k, v in d.items():
        ...
        d[k] = s
    with open('user_location(latitude_longitude).txt', 'w') as f:
        for k, v in d.items():
            f.write('%s %f %f\n' % (k, v[0], v[1]))
...
if __name__ == '__main__':
    split_training_data('dataAnalysis/training.txt', 1000, 100)